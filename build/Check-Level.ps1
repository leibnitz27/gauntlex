...
Get-ChildItem (Join-Path $Root 'levels') -Filter '*.txt' | ForEach-Object {
    ...
    for ($r = 0; $r -lt $H; $r++) { for ($c = 0; $c -lt $W; $c++) {
        ...
        if ('KDX+G'.Contains($ch)) { $cells[$ch] += , @($r, $c) }
    } }
    ...
    # keys / food / grunts must be reachable WITHOUT opening doors (no key yet / grunts can't open)
    Report 'K' 'key'   $reachNoDoor
    Report '+' 'food'  $reachNoDoor
    Report 'G' 'grunt' $reachNoDoor
    # exit must be reachable once doors can be opened
    Report 'X' 'exit'  $reachDoor
    # every door must border the pre-door reachable area (so you can open it)
    foreach ($p in $cells['D']) {
        $adj = $false
        foreach ($d in @(@(-1,0),@(1,0),@(0,-1),@(0,1))) {
            $r = $p[0]+$d[0]; $c = $p[1]+$d[1]
            if ($r -ge 0 -and $r -lt $H -and $c -ge 0 -and $c -lt $W -and $reachNoDoor[$r,$c]) { $adj = $true }
        }
        if ($adj) { Write-Host "  ok   door ($($p[0]),$($p[1])) approachable" }
        else      { Write-Host "  FAIL door ($($p[0]),$($p[1])) not approachable"; $script:fail++ }
    }
}
